...
if (lipStyle === "russian") {
  ...
  // Give the upper lip a slightly fuller
  // central arc without widening the corners.
  [39, 269].forEach((index) => {
    const point = result[index];
  ...
  // Round the lower lip instead of creating
  // a pointed center.
  const lowerLeft =
    result[84];
  ...
  // Anchor corners so the lips rise vertically
  // instead of becoming wider.
  [61, 291].forEach((index) => {
    const originalPoint =
      landmarks[index];
